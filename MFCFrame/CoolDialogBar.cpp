...
BOOL CCoolDialogBar::Create(CWnd* pParentWnd, CDialog *pDialog, CString &pTitle, UINT nID, DWORD dwStyle)
{
	...
	// create the child dialog
	m_cDialog = pDialog;
	BOOL bReturn = CControlBar::Create(AfxRegisterWndClass(CS_DBLCLKS, LoadCursor(NULL, IDC_ARROW), m_brushBkgd, 0), pTitle, dwStyle, CRect(0, 0, 0, 0), pParentWnd, nID);//, pContext); 
...
void CCoolDialogBar::DrawGripper(CDC & dc)
{
	...
	if (m_dwStyle & CBRS_ORIENT_HORZ) {
		...
		dc.DrawFrameControl(m_rectUndock, DFC_CAPTION, DFCS_CAPTIONRESTORE);
